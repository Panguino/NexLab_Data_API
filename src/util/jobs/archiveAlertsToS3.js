...
async function archiveAlertsToS3(cache) {
  try {
    ...
    // Extract alerts snapshot
    const snapshot = extractAlertsSnapshot(regionData);

    // Generate S3 key with date/time hierarchy
    const now = new Date();
    ...
    const s3Key = `alerts/${year}/${month}/${day}/${hour}-${minute}-${second}.json`;
    ...
    console.log(`✅ Archived ${snapshot.alerts_count} alerts to S3: ${s3Key}`);
...
/**
 * Extracts alerts from region data into a snapshot format
 * @param {Object} regionData - Cached region data
 * @returns {Object} Snapshot object with alerts organized by region
 */
function extractAlertsSnapshot(regionData) {
  ...
  try {
    ...
    for (const [regionName, region] of Object.entries(regionData)) {
      ...
      // Add region to snapshot if it has alerts
    ...
    console.error('Error extracting alerts snapshot:', error);
  }
